...
import { Body, Controller, Post, Route, Tags, Request, Res, TsoaResponse, Response } from 'tsoa';
...
@Route('/')
@Tags('USSD')
export class UssdController extends Controller {

    @Post('/ussd')
    @Response<String>(200, "CON Whoooo")
    public async initiateUssd(@Request() request: express.Request, @Res() res: TsoaResponse<200, string>): Promise<void> {
        logging.info(NAMESPACE, `Yay!`);
        console.log(request.body);
        const { sessionId, serviceCode, phoneNumber, text } = request.body;
        ...
        res(200, response, { 'Content-Type': 'text/plain' });
