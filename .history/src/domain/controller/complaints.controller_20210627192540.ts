...
@Tags('Complaints')
export class ComplaintsController extends Controller {
    ...
    @Get('/all/by/{userId}')
    public async getAllByUser(@Path() userId, @Query() limit?: number, @Res() notFoundResponse?: TsoaResponse<404, IErrorResponse>): Promise<IApiResponse> {
        logging.info(NAMESPACE, 'Get all complaints by user');

        let complaints: any[] = await ComplaintsService.listAllUserComplaints(userId, limit);
        if (!complaints || complaints.length == 0) notFoundResponse(404, { message: "No records found" });
